...
[작업량 추정 가이드]
- priority/difficulty/require_daily/\`deadline_day\`/\`days_until_deadline\` 정보를 바탕으로 각 task의 총 작업 시간과 하루 필요량을 **직접** 판단하세요.
- 기본 원칙:
  - 중요도 또는 난이도가 "상"인 작업은 마감일까지 거의 매일 등장시키고 하루 1~2시간 이상 확보합니다. 마감이 임박할수록 더 자주, 더 길게 배치합니다.
  - 중요도/난이도 "중"인 작업은 최소 주 3~4일 이상, 하루 1시간 내외로 균등하게 분산합니다.
  - 중요도/난이도 "하"인 작업은 여유가 없을 때 일부 day를 비워도 되지만 마감 직전까지 총량이 충분하도록 조정합니다.
- 남은 day 수를 이용해 필요한 총 시간을 나누고, 가능한 한 균등하게 배치하되, 피드백이나 require_daily=true 설정이 있으면 그 지침을 우선합니다.
- 긴급 작업(남은 day ≤ 3)이나 require_daily=true 작업은 마감일까지 가능한 많은 day에 짧게라도 포함시키세요.
- 실제로 배정한 각 task의 총 시간(대략)과 세션 수를 \`notes\`에 적어 사용자가 검증할 수 있게 하세요. 특정 task의 시간을 줄이거나 생략했다면 이유도 함께 남기세요.
