...
[절대 규칙]
1) 생활패턴: \`lifestyleMappingText\`에 나열된 항목만 배치. 사용자가 입력하지 않은 생활패턴 금지. "평일"은 월~금에만.
2) Task: tasks JSON의 모든 작업을 반드시 포함. deadline_day 이후 배치 금지. 중요/난이도 "중" 또는 "상"은 마감일까지 여러 날에 반복 배치 (최소 3일). **하루에 task가 하나만 배치되는 것은 절대 금지. 빈 시간대가 충분하면 여러 task를 배치하세요.**
3) 출력: JSON 객체 한 개만. 마크다운/주석 금지. 루트 키는 "scheduleData"와 "notes"만.
...
[배치 절차]
1) lifestyle: \`lifestyleMappingText\`에 따라 day별로 먼저 배치. 지정된 day에만 배치.
2) appointment: \`deadline_day\` + \`deadline_time\`에 정확히 배치.
3) task 배치 (매우 중요):
   - **하루에 여러 task 배치: 하루에 task가 하나만 배치되는 것은 절대 금지입니다. 빈 시간대가 충분하면 여러 task를 배치하세요.**
   - **우선순위 기반 배치:**
     * 중요도 "상" 또는 난이도 "상": 마감일까지 매일 배치, 하루 2시간 이상
     * 중요도 "중" 또는 난이도 "중": 마감일까지 최소 주 3~4일 이상, 하루 1시간 내외
     * 중요도 "하" 또는 난이도 "하": 마감일 직전까지 총량이 충분하도록 조정
